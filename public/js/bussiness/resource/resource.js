 ...
 var drawExpressions = function(expressions){
    ...
    if( expressions && expressions.length > 0){
        ...
        $('#expressions').html(html);
         ...
         $('#expressions').html('<h4>还没有表情，可以按照文件名规则上传</h4>');
    }
...
var drawExpressionsConstant = function(expressions){
    ...
    if( expressions && expressions.length > 0){
        ...
        for(var i=0;i<expressions.length;i++){
            ...
            html +='<a class="btn" onclick="delPartyResource(\''+expressions[i].id+'\',6)">删除</a><a class="btn" onclick="openFile(\''+expressions[i].id+'\',\''+6+'\')">小表情</a></div>';
        }
...
var drawSpecImages = function(specialImages){
    ...
    if( specialImages && specialImages.length > 0 ){
        ...
        $('#specialImages').html('<h4>还没有特效图片，可以按照文件名规则上传</h4>');
    }
}
    ...
    $.danmuAjax('/v1/api/admin/resource/page', 'GET','json',obj, function (data) {
        for(var i=0;i<data.rows.length;i++){
            ...
            if( i % 6  == 0){
                htmlStr += '<div>';
                if(isHave){
                    htmlStr += '<div style="display:inline-block;width:14%;border:1px solid red;"><img style="width:90%" src="'+fileUrl+'" /><p style="padding-left:10px"><a href="javascript:void(0);" class="btn" onclick="delResource(\''+data.rows[i].id+'\','+fileType+')">删除</a></p></div>';
                }else{
                ...
                if(isHave){
                    htmlStr += '<div style="display:inline-block;width:14%;margin-left:15px;border:1px solid red;"><img style="width:90%"  src="'+fileUrl+'" /><p style="padding-left:10px"><a href="javascript:void(0);" class="btn" onclick="delResource(\''+data.rows[i].id+'\','+fileType+')">删除</a></p></div>';
                }else{
